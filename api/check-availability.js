...
const CONFIG = {
  ...
  ORCHESTRATOR_FOLDER_KEY: '12a1d01a-457f-427a-a3eb-5b35a5078f1d',  // Modern folder key
...
async function getJobOutputByCorrelationId(correlationId, retryCount = 0) {
  ...
  const filter = `?$filter=Key eq '${correlationId}'`;
  ...
  const response = await fetch(jobsUrl + filter, {
    ...
    headers: {
      ...
      'X-UIPATH-FolderKey': CONFIG.ORCHESTRATOR_FOLDER_KEY
    }
  });
