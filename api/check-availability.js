...
const CONFIG = {
  ...
  INITIAL_POLL_DELAY: 500  // Faster initial poll
};
...
async function startJob(inputData) {
  ...
  const jobId = data.value[0].Id;  // Use numeric Id instead of GUID Key
...
async function pollJobCompletion(jobId, attempt = 0) {
  ...
  // Use numeric Id directly in URL path - much simpler than OData filter
  ...
  // Direct ID lookup returns the job object directly (not wrapped in .value array)
  const job = data;
  ...
  if (job.State === 'Successful') {
    ...
    // Exponential backoff: 0.5s, 1s, 2s, 3s, 3s...
    const delay = Math.min(CONFIG.INITIAL_POLL_DELAY * Math.pow(2, attempt), 3000);
...
module.exports = async (req, res) => {
  ...
  if (requestCache.has(requestKey)) {
    ...
    if (now - cached.timestamp < 3000) { // 3 second window
  ...
  try {
    ...
    // Cache the result for 5 seconds to prevent duplicate processing
    requestCache.set(requestKey, { result, timestamp: now });
