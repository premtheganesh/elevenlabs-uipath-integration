...
async function getLatestJobOutput(processName, retryCount = 0) {
  ...
  const filter = `?$filter=Release/Name eq '${processName}' and State eq 'Successful'&$orderby=EndTime desc&$top=1`;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const output = await getLatestJobOutput('Solution.3.api.API.Workflow');
